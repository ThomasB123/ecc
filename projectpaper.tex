...
%such as NSA Suite B (2005) and ANSSI FRP256V1 (2011). 
%Each of these standards tries to ensure that the ECDLP (finding a user's private key, given their public key) is difficult. 
%Unfortunately, there is a gap between ECDLP difficulty and ECC security, since there are many attacks which break real-world ECC systems 
%without solving the ECDLP such as side-channel attacks. 
%According to Bernstein \citeyear{bernstein2013safecurves}, none of these standards do a good job of ensuring ECC security, 
%secure implementations of the standard curves are theoretically possible but very hard. 

%Bernstein \citeyear{10.1007/11745853_14} designed a new high-security curve ECDH function which achieved record setting speeds 
%with several side benefits including state-of-the-art timing-attack protection. 
%This function, known as Curve25519, is deemed to be a ``safe curve" according to Bernstein \citeyear{bernstein2013safecurves}. 
%suitable for a wide variety of cryptographic applications. 
%The Curve25519 function is $F_p$-restricted x-coordinate scalar multiplication on $E(F_{p^2})$, 
%where $p$ is the prime number $2^{255} - 19$ and $E$ is the elliptic curve $y^2 = x^3 + 486662x^2 + x$. 
...
%In order to ensure that a curve is safe in the real-world, we require a set of criteria known as the Safe Curves criteria.
%If a curve meets this set of criteria, it is deemed a ‘safe curve’.
%In my project I am implementing an Elliptic Curve Cryptosystem using only ‘safe curves’.
%
